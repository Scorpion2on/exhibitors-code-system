...
function form_shortcode() {
    ...
    <style>
        ...
        <h1><?php echo get_option('h1_heading'); ?>Wpisz kod zaproszenia, który otrzymałeś</h1>
        <h3><?php echo get_option('h3_heading'); ?>Zostaniesz przekierowany wtedy do formularza rejestracji</h3>
            <form id="check_user" method="GET" action="">
                <input style="text-transform: uppercase;" type="text" id="input_value" name="input_value" />
                <button type="submit" class="btn btn-success"><?php echo get_option('button_text'); ?>WYŚLIJ</button>
            </form>
        </div>
    <?php
    return ob_get_clean();
